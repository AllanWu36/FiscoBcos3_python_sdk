...
def demo_data_market():
    try:
        ...
        client1 = Bcos3Client(client_config_otherone)
        
        # Get Sender Address
        # Fix for: type object 'client_config' has no attribute 'account_key'
        # 从底层 C SDK 获取当前账户的公钥对应的地址指针
        address_p = client1.bcossdk.bcos_sdk_get_keypair_address(client1.keypair)
        # 将 C 字符串指针转换为 Python 字符串（UTF-8 解码）
        sender_address = ctypes.string_at(address_p).decode("utf-8")
        # 释放底层 C SDK 分配的内存，防止内存泄漏
        client1.bcossdk.bcos_sdk_c_free(address_p)
        # 将地址转换为 Checksum 地址格式 (EIP-55)，以兼容 Web3.py 的校验要求
        client1_address = to_checksum_address(sender_address)
        print("client1's Address:", client1_address)

        client_config_otherone.account_keyfile = "allanwu36.keystore"
        client_config_otherone.account_password = "7758258w77"

        client = Bcos3Client(client_config_otherone)
        ...
        # Check balance again
        res = client.call(token_address, parser_token.contract_abi, "balances", [sender_address])
        print("client Balance after transfer:", res)

        res = client1.call(token_address, parser_token.contract_abi, "balances", [client1_address])
        print("client1 Balance after transfer:", res)

        recipient = sender_address
        print(f"-> Transferring 99 to {recipient}...")
        receipt = client1.sendRawTransaction(token_address, parser_token.contract_abi, "transfer", [recipient, 99])
        if receipt['status'] == 0:
             print("Transfer success. TxHash:", receipt['transactionHash'])
        else:
             print("Transfer failed:", receipt)
